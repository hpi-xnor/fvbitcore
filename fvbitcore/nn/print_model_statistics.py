...
def _get_quantization_keys(model: nn.Module, quantization_base_class: Type = None) -> Dict[str, int]:
    keys = {}
    if quantization_base_class is None:
        return keys
    for name, module in model.named_modules():
        variables = vars(module)
        quantizations = [variable for variable in variables if issubclass(type(variable), quantization_base_class)]
...
def flop_count_table(
    ...
    if automatic_qmodules:
        quantized_modules = _get_quantization_keys(flops._model, flops._quantization_base_class)
